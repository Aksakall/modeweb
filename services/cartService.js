import { apiRequest, getAccessToken } from './apiClient.js';
...
export const cartService = {
  ...
  add:async payload=>{
    if(!useLocalCart()) return mutateRemote('/api/cart/items',{method:'POST',body:payload});
  ...
  update:async(id,payload)=>{ if(!useLocalCart()) return mutateRemote(`/api/cart/items/${id}`,{method:'PATCH',body:{quantity:Number(payload.quantity)}}); const items=read(); const item=items.find(i=>i.id===id); if(item)item.quantity=Math.max(1,Math.min(item.stock,payload.quantity)); return write(items); },
  remove:async id=>{ if(!useLocalCart()) return mutateRemote(`/api/cart/items/${id}`,{method:'DELETE'}); return write(read().filter(i=>i.id!==id)); },
  clear:()=>useLocalCart() ? Promise.resolve(write([])) : apiRequest('/api/cart/clear',{method:'DELETE'}),
