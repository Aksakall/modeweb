...
function productForm(product={}){
  const colors=normalizeColors(product.colors);const sizes=normalizeSizes(product.sizes,product.variants);const variants=normalizeVariants(product.variants,product.id);
  return `<form class="admin-form" id="product-form" data-id="${escapeHtml(product.id||'')}"><div class="form-grid"><label>Ürün Adı<input name="name" value="${escapeHtml(product.name||'')}" required></label><label>Slug<input name="slug" value="${escapeHtml(product.slug||'')}" required></label><label>SKU<input name="sku" value="${escapeHtml(product.sku||'')}" required></label><label>Marka<input name="brand" value="${escapeHtml(product.brand||'Sıla Sarıoğlu')}" required></label><label>Kategori Slug<input name="categorySlug" value="${escapeHtml(product.categorySlug||'')}" required></label><label>Alt Kategori Slug<input name="subCategorySlug" value="${escapeHtml(product.subCategorySlug||'')}" required></label><label>Fiyat<input name="price" type="number" min="0" step="0.01" value="${product.price??''}" required></label><label>Eski Fiyat<input name="oldPrice" type="number" min="0" step="0.01" value="${product.oldPrice??''}"></label><label>İndirim Oranı<input name="discountPercent" type="number" min="0" max="100" value="${product.discountPercent??0}"></label><label>Toplam Stok<input name="stock" type="number" min="0" value="${product.stock??0}" required></label><label>Kritik Stok<input name="criticalStock" type="number" min="0" value="${product.criticalStock??5}"></label><label>Durum<select name="status"><option value="active" ${product.status!=='passive'?'selected':''}>Aktif</option><option value="passive" ${product.status==='passive'?'selected':''}>Pasif</option></select></label><label class="full">Görseller <small>Her satıra bir URL</small><textarea name="images" required>${escapeHtml((product.images||[]).join('\n'))}</textarea></label><label class="full">Thumbnail<input name="thumbnail" value="${escapeHtml(product.thumbnail||'')}" required></label>${optionBuilder('Renkler','Renk Ekle','color',colors)}${optionBuilder('Bedenler','Beden Ekle','size',sizes)}${variantBuilder(variants)}<label class="full">Açıklama<textarea name="description">${escapeHtml(product.description||'')}</textarea></label><label>Kumaş Bilgisi<textarea name="fabricInfo">${escapeHtml(product.fabricInfo||'')}</textarea></label><label>Ölçüler<textarea name="measurements">${escapeHtml(product.measurements||'')}</textarea></label><label class="full">Bakım Talimatı<textarea name="careInstructions">${escapeHtml(product.careInstructions||'')}</textarea></label><fieldset class="full check-grid"><label><input type="checkbox" name="isFeatured" ${product.isFeatured?'checked':''}> Öne Çıkan</label><label><input type="checkbox" name="isDiscounted" ${product.isDiscounted?'checked':''}> İndirimli</label><label><input type="checkbox" name="isNew" ${product.isNew?'checked':''}> Yeni</label><label><input type="checkbox" name="isBestSeller" ${product.isBestSeller?'checked':''}> Çok Satan</label><label><input type="checkbox" name="freeShipping" ${product.freeShipping?'checked':''}> Ücretsiz Kargo</label></fieldset><label>SEO Başlık<input name="seoTitle" value="${escapeHtml(product.seoTitle||'')}"></label><label>SEO Açıklama<textarea name="seoDescription">${escapeHtml(product.seoDescription||'')}</textarea></label></div><div class="modal-actions"><button type="button" class="secondary" data-modal-close>Vazgeç</button><button type="submit">${product.id?'Ürünü Güncelle':'Ürünü Oluştur'}</button></div></form>`;
}

function normalizeColors(colors=[]){return colors.length?colors.map((color,index)=>({id:color.id||`color-${index+1}`,name:color.name||'',hex:color.hex||'#6f1d24'})):[{id:'color-1',name:'',hex:'#6f1d24'}]}
function normalizeSizes(sizes=[],variants=[]){const values=sizes.length?sizes:[...new Set((variants||[]).map(item=>item.size).filter(Boolean))];return values.length?values:['']}
function normalizeVariants(variants=[],productId=''){return variants.length?variants.map((variant,index)=>({id:variant.id||'',productId:variant.productId||productId,color:variant.color||'',size:variant.size||'',sku:variant.sku||'',stock:variant.stock??0,price:variant.price??''})):[{id:'',productId,color:'',size:'',sku:'',stock:0,price:''}]}
function colorRow(color={}){return `<div class="builder-row color-row" data-builder-row><input name="colorName" placeholder="Renk adı" value="${escapeHtml(color.name||'')}" required><input name="colorHex" type="color" value="${escapeHtml(color.hex||'#6f1d24')}" aria-label="Renk kodu"><button type="button" class="ghost" data-remove-row aria-label="Rengi sil">Sil</button></div>`}
function sizeRow(size=''){return `<div class="builder-row size-row" data-builder-row><input name="sizeValue" placeholder="Beden / ölçü" value="${escapeHtml(size)}" required><button type="button" class="ghost" data-remove-row aria-label="Bedeni sil">Sil</button></div>`}
function variantRow(variant={}){return `<div class="builder-row variant-row" data-builder-row data-variant-id="${escapeHtml(variant.id||'')}"><input name="variantColor" placeholder="Renk" value="${escapeHtml(variant.color||'')}" required><input name="variantSize" placeholder="Beden" value="${escapeHtml(variant.size||'')}" required><input name="variantSku" placeholder="Varyant SKU" value="${escapeHtml(variant.sku||'')}"><input name="variantStock" type="number" min="0" placeholder="Stok" value="${variant.stock??0}" required><input name="variantPrice" type="number" min="0" step="0.01" placeholder="Fiyat" value="${variant.price??''}"><button type="button" class="ghost" data-remove-row aria-label="Varyantı sil">Sil</button></div>`}
function optionBuilder(title,button,type,items){const rows=type==='color'?items.map(colorRow).join(''):items.map(sizeRow).join('');return `<fieldset class="full option-builder" data-builder="${type}"><legend>${title}</legend><div class="builder-list" data-builder-list>${rows}</div><button type="button" class="secondary compact" data-add-${type}>${button}</button></fieldset>`}
function variantBuilder(variants){return `<fieldset class="full option-builder variant-builder" data-builder="variant"><legend>Varyant / Stok</legend><p>Her renk-beden kombinasyonu için stok ve gerekiyorsa özel fiyat girin.</p><div class="builder-list" data-builder-list>${variants.map(variantRow).join('')}</div><button type="button" class="secondary compact" data-add-variant>Varyant/Stok Ekle</button></fieldset>`}
function collectRows(form,selector,mapper){return [...form.querySelectorAll(selector)].map(mapper).filter(Boolean)}
function serializeProduct(form){const data=new FormData(form);const basePrice=Number(data.get('price'));const colors=collectRows(form,'.color-row',row=>{const name=row.querySelector('[name="colorName"]').value.trim();if(!name)return null;return {id:createSlug(name)||rowId(),name,hex:row.querySelector('[name="colorHex"]').value||'#6f1d24'}});const sizes=collectRows(form,'.size-row',row=>row.querySelector('[name="sizeValue"]').value.trim()).filter(Boolean);const variants=collectRows(form,'.variant-row',row=>{const color=row.querySelector('[name="variantColor"]').value.trim();const size=row.querySelector('[name="variantSize"]').value.trim();if(!color||!size)return null;return {id:row.dataset.variantId||undefined,productId:form.dataset.id||undefined,color,size,sku:row.querySelector('[name="variantSku"]').value.trim(),stock:Number(row.querySelector('[name="variantStock"]').value||0),price:row.querySelector('[name="variantPrice"]').value?Number(row.querySelector('[name="variantPrice"]').value):basePrice};});if(!colors.length)throw new Error('En az bir renk ekleyin.');if(!sizes.length)throw new Error('En az bir beden ekleyin.');if(!variants.length)throw new Error('En az bir varyant/stok satırı ekleyin.');return {name:data.get('name').trim(),slug:data.get('slug').trim()||createSlug(data.get('name')),sku:data.get('sku').trim(),brand:data.get('brand').trim(),categorySlug:data.get('categorySlug').trim(),subCategorySlug:data.get('subCategorySlug').trim(),price:basePrice,oldPrice:data.get('oldPrice')?Number(data.get('oldPrice')):null,discountPercent:Number(data.get('discountPercent')||0),images:data.get('images').split(/\r?\n/).map(value=>value.trim()).filter(Boolean),thumbnail:data.get('thumbnail').trim(),colors,sizes,variants,stock:Number(data.get('stock')),criticalStock:Number(data.get('criticalStock')||0),description:data.get('description').trim(),fabricInfo:data.get('fabricInfo').trim(),measurements:data.get('measurements').trim(),careInstructions:data.get('careInstructions').trim(),isFeatured:data.has('isFeatured'),isDiscounted:data.has('isDiscounted'),isNew:data.has('isNew'),isBestSeller:data.has('isBestSeller'),freeShipping:data.has('freeShipping'),status:data.get('status'),seoTitle:data.get('seoTitle').trim(),seoDescription:data.get('seoDescription').trim()};}
...
document.addEventListener('click',async event=>{
  ...
  if(event.target.closest('[data-add-color]')){event.target.closest('[data-builder]').querySelector('[data-builder-list]').insertAdjacentHTML('beforeend',colorRow());return}
  if(event.target.closest('[data-add-size]')){event.target.closest('[data-builder]').querySelector('[data-builder-list]').insertAdjacentHTML('beforeend',sizeRow());return}
  if(event.target.closest('[data-add-variant]')){event.target.closest('[data-builder]').querySelector('[data-builder-list]').insertAdjacentHTML('beforeend',variantRow());return}
  const removeRow=event.target.closest('[data-remove-row]');if(removeRow){const list=removeRow.closest('[data-builder-list]');if(list.children.length>1)removeRow.closest('[data-builder-row]').remove();return}
